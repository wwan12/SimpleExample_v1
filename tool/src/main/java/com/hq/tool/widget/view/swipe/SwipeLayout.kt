...
class SwipeLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : FrameLayout(context, attrs, defStyle) {
    ...
    private var mShowMode: ShowMode=ShowMode.LayDown
    ...
    private val mViewBoundCache: MutableMap<View?, Rect> = HashMap() //save all children's bound, restore in onLayout
     ...
     * [com.hq.tool.model.swipe.SwipeLayout.OnRevealListener]
     *
     ...
     * @param l        the [com.hq.tool.model.swipe.SwipeLayout.OnRevealListener]
     */
        ...
        override fun onViewPositionChanged(changedView: View, left: Int, top: Int, dx: Int, dy: Int) {
            ...
            if (changedView === surfaceView) {
                ...
                if (mShowMode == ShowMode.PullOut) {
                    ...
                    if (dragEdge == DragEdge.Left && newLeft < paddingLeft) newLeft = paddingLeft else if (dragEdge == DragEdge.Right && newLeft > paddingLeft) newLeft = paddingLeft else if (dragEdge == DragEdge.Top && newTop < paddingTop) newTop = paddingTop else if (dragEdge == DragEdge.Bottom && newTop > paddingTop) newTop = paddingTop
    ...
    protected fun dispatchRevealEvent(surfaceLeft: Int, surfaceTop: Int, surfaceRight: Int,
        ...
        for ((child, value) in mRevealListeners) {
            ...
            if (isViewShowing(child, rect, dragEdge, surfaceLeft, surfaceTop,
            ...
            if (isViewTotallyFirstShowed(child, rect, dragEdge, surfaceLeft, surfaceTop,
                            surfaceRight, surfaceBottom)) {
                mShowEntirely[child] = true
                for (l in value!!) {
                    if (dragEdge == DragEdge.Left
                            || dragEdge == DragEdge.Right) l.onReveal(child, dragEdge, 1f, child.width) else l.onReveal(child, dragEdge, 1f, child.height)
                }
            }
        }
    }
    ...
    private fun performAdapterViewItemClick() {
        ...
        if (t is AdapterView<*>) {
            ...
            if (p != AdapterView.INVALID_POSITION) {
                view.performItemClick(view.getChildAt(p - view.firstVisiblePosition), p, view
                        .adapter.getItemId(p))
            }
        }
    }
        ...
        if (t is AdapterView<*>) {
            ...
            try {
                ...
                if (view.onItemLongClickListener != null) {
                    handled = view.onItemLongClickListener.onItemLongClick(view, this@SwipeLayout, p, vId)
                }
                if (handled) {
     ...
     * [com.hq.tool.model.swipe.SwipeLayout.ShowMode].PullOut and
     * [com.hq.tool.model.swipe.SwipeLayout.ShowMode].LayDown.
     *
     * @param mode
     */
     ...
     * get the open status.
     *
     * @return [com.hq.tool.model.swipe.SwipeLayout.Status] Open , Close or
     * Middle.
     */
    val openStatus: Status
        get() {
            ...
            return if (surfaceLeft == paddingLeft - mDragDistance || surfaceLeft == paddingLeft + mDragDistance || surfaceTop == paddingTop - mDragDistance || surfaceTop == paddingTop + mDragDistance) Status.Open else Status.Middle
        }
    ...
    fun toggle(smooth: Boolean = true) {
        if (openStatus == Status.Open) close(smooth) else if (openStatus == Status.Close) open(smooth)
    }
    ...
    private fun computeSurfaceLayoutArea(open: Boolean): Rect {
        ...
        var t = paddingTop
        if (open) {
            if (dragEdge == DragEdge.Left) l = paddingLeft + mDragDistance else if (dragEdge == DragEdge.Right) l = paddingLeft - mDragDistance else if (dragEdge == DragEdge.Top) t = paddingTop + mDragDistance else t = paddingTop - mDragDistance
        }
    ...
    private fun computeBottomLayoutAreaViaSurface(mode: ShowMode, surfaceArea: Rect?): Rect {
        ...
        if (mode == ShowMode.PullOut) {
            if (dragEdge == DragEdge.Left) bl = surfaceArea.left - mDragDistance else if (dragEdge == DragEdge.Right) bl = surfaceArea.right else if (dragEdge == DragEdge.Top) bt = surfaceArea.top - mDragDistance else bt = surfaceArea.bottom
            if (dragEdge == DragEdge.Left || dragEdge == DragEdge.Right) {
            ...
            if (dragEdge == DragEdge.Left) br = bl + mDragDistance else if (dragEdge == DragEdge.Right) bl = br - mDragDistance else if (dragEdge == DragEdge.Top) bb = bt + mDragDistance else bt = bb - mDragDistance
        }
    ...
    fun setDragEdges(vararg mDragEdges: DragEdge?) {
        clearDragEdge()
        dragEdges = listOf(mDragEdges) as List<DragEdge>
    }
